...
public class InventoryPanel extends JPanel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Handle the consume button 
        if (source == consumeButton) {
            int selectIndex = consumeList.getSelectedIndex();
            if (selectIndex != -1) {
                inventory.useItem(selectIndex);
            }
        }
    
        // Handles equip button action
        for (InventorySlotType type : InventorySlotType.values()) {
            JButton equipButton = equipButtons.get(type);
            if (source == equipButton) {
                int selectedIndex = equipLists.get(type).getSelectedIndex();
                if (selectedIndex != -1) {
                    EquippableItem itemToEquip = inventory.getItemForSlot(type, selectedIndex);
